...
// Possible answer: could leverage what I know about ValueModifier and expect
// solve() to return val()**3
// - Unit test ValueModifier independently
// - Unit test Solver that depends on ValueModifier
//
// What if I change the underlying behavior of my ValueModifier to instead
// calculate the fibbonacci?
...
{
 public:
  explicit Solver(const int clipping_limit, std::unique_ptr<IValueModifier> value_modifier_ptr)
...
// - simply create a different implementation of IValueModifier which performs
// the fibbonacci
//
// How can I create my concrete object?
//
// TODO: incorporate rules of Dependency Inversion Principle (avoiding concrete
...
class ValueModifierFactory
{
 public:
  ...
  static std::unique_ptr<IValueModifier>
  makeValueModifier(const ModifierType& mod_type)
  {
    if (mod_type == SQUARE)
    {
